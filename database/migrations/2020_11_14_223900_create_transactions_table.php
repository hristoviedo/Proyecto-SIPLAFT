...
class CreateTransactionsTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('transactions', function (Blueprint $table) {
            ...
            $table->string('apartment_number'); //Número de apartamento (not null)
            $table->string('intermediary_bank')->default("CONTADO")->nullable(); //Número de apartamento (not null)
            $table->string('operation_date')->nullable(); //Fecha en que registró la operación (not null)
            $table->string('transfer_date')->nullable(); //Fecha en que registró el traspaso de escritura (not null)
            $table->boolean('cash')->nullable(); //¿La transacción se hizo en efectivo? True si es verdadero (not null)
            $table->string('currency')->nullable(); //Tipo de moneda en que se hizo la transacción (not null)
            $table->unsignedDecimal('amount',12,2); //Monto de la transacción (not null)
